    ...
    public class DataBaseAdapter : ConexaoBanco
    {
        ...
        public void criarTables(string nomeBD)
        {
            try
            {
                ...
                " CREATE TABLE IF NOT EXISTS " + nomeBD + ".`viewlistarlocacaoitensdev` (`IDProduto` INT, `IDProdutoVariacao` INT, `Descricao` INT, `UN` INT, `ValorLocado` INT, `QtdLocada` INT, `QtdDevolvida` INT, `Restante` INT, `Marca` INT, `Categoria` INT, `SubCategoria` INT, `Fornecedor` INT, `Cod.Fabricante` INT, `Cod.Interno` INT, `CodBarras` INT, `IDLocacao` INT); "+
