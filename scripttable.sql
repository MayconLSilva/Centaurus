...
CREATE TABLE IF NOT EXISTS `centaurus`.`cidade` (
  ...
  `codigo_cidade` INT(11) NULL DEFAULT NULL COMMENT '',
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `centaurus`.`viewlistarlocacaoitensdev` (`IDProduto` INT, `IDProdutoVariacao` INT, `Descricao` INT, `UN` INT, `ValorLocado` INT, `QtdLocada` INT, `QtdDevolvida` INT, `Restante` INT, `Marca` INT, `Categoria` INT, `SubCategoria` INT, `Fornecedor` INT, `Cod.Fabricante` INT, `Cod.Interno` INT, `CodBarras` INT, `IDLocacao` INT);
...
FOR EACH ROW
BEGIN  delete from locacaoitens where idLocacao_locacaoitens = OLD.id_locacao;                 END$$

USE `centaurus`$$
CREATE
DEFINER=`sistema`@`%`
TRIGGER `centaurus`.`Estoque_locacaoitens_Delete`
AFTER DELETE ON `centaurus`.`locacaoitens`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`sistema`@`%`
TRIGGER `centaurus`.`Estoque_locacaoitens_Insert`
AFTER INSERT ON `centaurus`.`locacaoitens`
FOR EACH ROW
BEGIN
UPDATE produto SET saldo_produto = saldo_produto - NEW.qtdLocada_locacaoitens WHERE id_produto = NEW.idProduto_locacaoitens;
